...
public class LevelSystem : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        startImage = levelPanel.transform.Find("Star").GetComponent<Image>();
        lvlText = startImage.transform.GetChild(0).GetComponent<TextMeshProUGUI>(); ;
    ...
    private void Start()
    {
        EventManager.Instance.AddListener<XPAddedGameEvent>(OnXPAddded);
    ...
    private void OnXPAddded(XPAddedGameEvent info)
    {
        XPNow += info.amount;
        UpdateUI();
        if(XPNow >= xpToNext)
    ...
    private void OnLevelChanged(LevelChangedGameEvent info)
    {
        ...
        GameObject window = Instantiate(lvlWindowPrefab, HomeManager.current.canvas.transform);
